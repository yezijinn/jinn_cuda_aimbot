...
namespace makcu {
    ...
    public:
        ...
        // 回收连接监控线程（幂等，未启动/已回收时为空操作）
        void cleanupMonitoringThread() {
            // 防御：监控线程不能回收自己（自 join 会抛 resource_deadlock_would_occur）。
            // 当前调用链不会走到这里，但若将来有人从掉线回调里调 disconnect()，
            // 这里直接跳过比抛异常/崩溃更安全。
            if (monitoringThread.get_id() == std::this_thread::get_id()) {
                return;
            }
            ...
            // 置停止标志，release 序保证此前的状态写对监控线程可见
            stopMonitoring.store(true, std::memory_order_release);

            // 持锁 notify：避免监控线程"判完谓词、尚未进入 wait"这一窗口内丢失唤醒
            {
                std::lock_guard<std::mutex> lock(monitoringMutex);
                monitoringCondition.notify_all();
            }

            // 直接 join。此处原先使用 std::async + wait_for(2000ms) + detach 的"伪超时"方案，
            // 存在三重缺陷，全部会导致进程级故障：
            //   1) 超时是假的：std::async(std::launch::async) 返回的 future 在析构时必须阻塞
            //      到任务结束，wait_for 超时后函数返回，future 析构照样无限期死等，
            //      所谓"防止无限阻塞"完全没有生效；
            //   2) 数据竞争：超时分支的 detach() 与 async 线程里的 join() 并发操作同一个
            //      std::thread 对象，是未定义行为，实测可直接触发 std::terminate；
            //   3) UAF：detach 之后若 Impl 被销毁，仍在运行的监控线程会继续写已释放内存。
            // connectionMonitoringLoop 的等待点是带谓词的 condition_variable::wait_for，
            // 收到上面的 notify_all 后必然在一个检查周期内返回，join 不会长时间阻塞。
            // 同一模块的 SerialPort::closeLocked 此前已按同样理由改为直接 join。
    ...
    bool Device::connect(const std::string& port) {
        ...
        // 先回收上一轮可能残留的监控线程，再重置停止标志。
        // 原因：connectionMonitoringLoop 检测到设备掉线时会自行 break 返回（见该函数内的两处
        // break），线程函数虽已结束，但 std::thread 对象依然处于 joinable 状态，而这条路径上
        // 没有任何配套的 join。用户随后重连时，下面的
        //     monitoringThread = std::thread(...)
        // 就是对一个 joinable 的 std::thread 做移动赋值 —— 按 C++ 标准会调用 std::terminate，
        // 整个进程被直接杀掉，且没有任何日志。这是"拔掉 MAKCU 再插回去重连"必崩的根因。
        // cleanupMonitoringThread 内部已做 joinable 判定，首次连接时是无副作用的空操作。
        ...
        try {
            ...
            // 异常对象未被使用，改为无名参数以消除 C4101 警告（行为不变）。
    ...
    void Device::disconnect() {
        ...
        // Close the serial port UNCONDITIONALLY before the CAS check.
        // The monitoring thread may have already flipped `connected` to false on
        // link drop; if we returned early on CAS failure without closing, the
        // SerialPort handle would leak, and a subsequent reconnect -> open() ->
        // close() would hit the recursive-mutex self-deadlock (P0-1). Closing
        // here guarantees the handle is released in every disconnect path.
        if (m_impl->serialPort->isOpen()) {
            m_impl->serialPort->close();
        }

        // Use compare_exchange to prevent race conditions with monitoring thread.
        // Only the connection-change *notification* is skipped when another thread
        // already performed the transition; the serial port is already closed above.
        bool expectedConnected = true;
        const bool weTransitioned =
            m_impl->connected.compare_exchange_strong(expectedConnected, false, std::memory_order_acq_rel);
        if (!weTransitioned) {
