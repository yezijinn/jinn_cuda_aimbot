...
static int KM_lock_device(void)
{
	...
	// 取锁后复检 fd：KM_close 与 KM_lock_device 经本互斥体串行化，若取锁前
	// KM_close 已将 fd_kmbox 置空/复用，此处可避免对空（或已复用）句柄 hid_write。
	if (fd_kmbox == NULL) {
...
int KM_close(void)
{
	// 仅关闭 HID 设备句柄；互斥体保留至进程退出由 OS 回收。
	// 原实现 CloseHandle(m_hMutex_lock) 会销毁一个可能正被其它线程
	// WaitForSingleObject 等待的内核对象（MSDN：行为未定义），且导致后续
	// ReleaseMutex 读到 NULL 静默失败、锁语义彻底丢失。保留互斥体后 KM_init
	// 复用同一句柄（无泄漏），且 KM_lock_device 在 fd 置空后能正确返回 -1。
...
int KM_init(unsigned short vid,unsigned short pid)
{
	...
	if(m_hMutex_lock==NULL)
	{
		m_hMutex_lock= CreateMutexA(NULL,FALSE,NULL);  // 匿名互斥体：避免 "busy" 通用名在 Local\ 命名空间被其它进程/实例串锁
		if (m_hMutex_lock == NULL) {
			return -1;
		}
	}
...
{		unsigned char buff[65]={0};
		...
		// 长度上界保护：str 超过 data[58] 时越界写（堆/全局缓冲区溢出）。
		size_t lcdCopy = strlen(str);
		if (lcdCopy > sizeof(cmd07_lcdstr.data) - 1)
			lcdCopy = sizeof(cmd07_lcdstr.data) - 1;
		memcpy(cmd07_lcdstr.data, str, lcdCopy);
		if (KM_lock_device() != 0) return -1;
...
{	kmbox_t km;
	...
	if (index < 1 || index > 5)   // script 数组固定 5 项；index 越界（含 0）直接拒绝，避免下标越界写
		return -1;
	if(	g_script_info.script[index-1].Exist==0)
	{
		...
		// 【第 28 轮修复·越界写】原实现为 memcpy(dst, name, strlen(name))，
		// 目标 Name 为 char[12]（kmboxA.h:13），而 strlen(name) 无任何上界：
		// 脚本名超过 12 字节即越界写入全局结构体 g_script_info，
		// 覆盖紧随其后的 Length / StartAddr 等字段，进而使 KM_download 用
		// 被污染的长度和地址写 flash（可损坏设备固件数据区）。name 亦未判空。
		// 修法：判空 + 按目标容量截断 + 强制补 '\0'，与第 24 轮 KM_LCDstr 的处理一致。
		{
			char* const nameField = g_script_info.script[index-1].Name;
			const size_t nameCapacity = sizeof(g_script_info.script[index-1].Name);
			memset(nameField, 0, nameCapacity);
			if (name != NULL)
			{
				size_t nameLen = strlen(name);
				if (nameLen > nameCapacity - 1)
					nameLen = nameCapacity - 1;
				memcpy(nameField, name, nameLen);//script name (truncated, always NUL-terminated)
			}
		}
